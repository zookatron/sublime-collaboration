...
class WebSocketApp(object):
    ...
    def send(self, data):
        ...
        """
        #if("\"op\"" in data): raise ValueError()
...
class ClientWebSocket(threading.Thread):
    ...
    def run(self):
        self.sock = WebSocket()
        self.sock.connect("ws://{0}:{1}".format(self.host, self.port))
        self.emit('open')
        while self.keep_running:
            data = self.sock.recv()
...
class ServerWebSocket(threading.Thread):
    ...
    def _handshake(self):
        ...
        headers.append("Upgrade: WebSocket")
        headers.append("Connection: Upgrade")
        headers.append("Sec-WebSocket-Accept: %s" % key)
        headers.append("Sec-WebSocket-Protocol: %s" % 'sample')
        headers.append("")
        headers.append("")
    ...
    def _validate_header(self, headers):
        return bool(headers.get("sec-websocket-key", None))
...
class WebSocketServer:
    ...
    def run_forever(self):
        ...
        while self.keep_running:
            ...
            connection = ServerWebSocket(conn, addr)
            ...
            connection.start()
            while not connection.ready(): pass
